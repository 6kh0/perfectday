...
import { PLAYER_DOWN, PLAYER_PALETTE, PLAYER_SIDE, PLAYER_UP, type Bitmap, type Palette } from "../src/game/sprites";

type RGBA = [number, number, number, number];
const CLEAR: RGBA = [0, 0, 0, 0];
const parseColor = (h: string): RGBA => [parseInt(h.slice(1, 3), 16), parseInt(h.slice(3, 5), 16), parseInt(h.slice(5, 7), 16), 255];

function compose(sprites: Bitmap[], palette: Palette, scale: number): RGBA[][] {
  const colors = Object.fromEntries(Object.entries(palette).map(([k, v]) => [k, parseColor(v)])) as Record<string, RGBA>;
  const h = Math.max(...sprites.map(s => s.length));
  const w = sprites.reduce((n, s) => n + s[0]!.length, 0);
  const pixels: RGBA[][] = Array.from({ length: h * scale }, () => Array.from({ length: w * scale }, () => CLEAR));
  let ox = 0;
  for (const s of sprites) {
    s.forEach((row, y) =>
      [...row].forEach((ch, x) => {
        const c = colors[ch];
        if (!c) return;
        for (let dy = 0; dy < scale; dy++) for (let dx = 0; dx < scale; dx++) pixels[y * scale + dy]![(ox + x) * scale + dx] = c;
      }),
    );
    ox += s[0]!.length;
  }
